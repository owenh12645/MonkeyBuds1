            ...
            foreach (MirrorSurface reflectionMs in mirrorSurfaces)
            {
                    ...
                    ) // && (depth != 2 || (depth == 2 && reflectionMs.VisibleInBoundsParent(parentSurface, _reflectionCamera, renderCamera)))
                {
                    ...
                    if (myDistance <= reflectionMs.maxRenderingDistance || reflectionMs.useRecursiveDarkening)
                    {
                        ...
                        cameraMatricesInOrder.Add(new CameraMatrices(renderCamera, newProjectionMatrix, newWorldToCameraMatrix, newCullingMatrix, reflectionMs, depth % 2 != 0, newEyePos, depth, myDistance, parentSurface, parentsParentSurface, parentsParentsParentSurface, parentsParentsParentsParentSurface));
                    }
                    else
                    {
                        //Debug.Log("to far: " + reflectionMs.name + " : " + depth + " : " + myDistance + " : " + reflectionMs.maxRenderingDistance);
                        cameraMatricesInOrder.Add(new CameraMatrices(renderCamera, Matrix4x4.identity, Matrix4x4.identity, Matrix4x4.identity, reflectionMs, true, Vector3.zero, recursions + 1, myDistance, parentSurface, parentsParentSurface, parentsParentsParentSurface, parentsParentsParentsParentSurface));
                    }
                }
            }
